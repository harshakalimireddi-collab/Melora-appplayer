...
title: TimeZone
description: Documentation for the TimeZone API for melora plugins
order: 3
---

The `TimeZone` API provides access to the current time zone of the device running melora. This can be useful for plugins that need to display
or handle time-related information based on the user's local time zone.

## Usage

To use the `TimeZone` API, you can import the `melora_plugin` module and access the `TimeZone` class.

```hetu_script
import "module:melora_plugin" as melora

var TimeZone = melora.TimeZone
```
