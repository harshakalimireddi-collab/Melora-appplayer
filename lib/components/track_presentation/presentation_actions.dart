...
import 'package:melora/components/dialogs/confirm_download_dialog.dart';
...
ToastOverlay showToastForAction(
  ...
  final message = switch (action) {
    "download" => (context.l10n.download_count(count), MeloraIcons.download),
    "add-to-playlist" => (
        context.l10n.add_count_to_playlist(count),
        MeloraIcons.playlistAdd
      ),
    "add-to-queue" => (
        context.l10n.add_count_to_queue(count),
        MeloraIcons.queueAdd
      ),
    "play-next" => (
        context.l10n.play_count_next(count),
        MeloraIcons.lightning
      ),
    _ => ("", MeloraIcons.error),
  };
...
class TrackPresentationActionsSection extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, ref) {
    ...
    Future<void> actionDownloadTracks({
      ...
      required List<MeloraTrackObject> tracks,
      required String action,
    }) async {
      final fullTrackObjects =
          tracks.whereType<MeloraFullTrackObject>().toList();
    ...
    return AdaptivePopSheetList(
          ...
          case "play-next":
            {
              ...
              if (options.collection is MeloraSimpleAlbumObject) {
                historyNotifier.addAlbums(
                    [options.collection as MeloraSimpleAlbumObject]);
              } else {
      ...
      icon: const Icon(MeloraIcons.moreVertical),
      variance: ButtonVariance.outline,
      items: (context) => [
        AdaptiveMenuButton(
          value: "download",
          leading: const Icon(MeloraIcons.download),
