...
final localTracksProvider =
    FutureProvider<Map<String, List<MeloraLocalTrackObject>>>((ref) async {
  try {
    if (kIsWeb) return {};
    final Map<String, List<MeloraLocalTrackObject>> libraryToTracks = {};
    ...
    for (final location in [
      ...
      final tracksFromMetadata = filesWithMetadata
          .map(
            (fileWithMetadata) => MeloraTrackObject.localTrackFromFile(
              fileWithMetadata.file,
              metadata: fileWithMetadata.metadata,
              art: fileWithMetadata.art,
            ) as MeloraLocalTrackObject,
          )
          .toList();
