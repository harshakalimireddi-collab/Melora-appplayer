...
class SourcedTrack extends BasicSourcedTrack {
  ...
  static List<MeloraAudioSourceMatchObject> rankResults(
    List<MeloraAudioSourceMatchObject> results,
  ...
  static Future<List<MeloraAudioSourceMatchObject>> fetchSiblings({
    required MeloraFullTrackObject query,
    required Ref ref,
  }) async {
    final audioSource = await ref.read(audioSourcePluginProvider.future);
    ...
    final videoResults = <MeloraAudioSourceMatchObject>[];
  ...
  Future<SourcedTrack?> swapWithSibling(
    MeloraAudioSourceMatchObject sibling,
  ) async {
  ...
  Future<SourcedTrack> refreshStream() async {
    ...
    List<MeloraAudioSourceStreamObject> validStreams = [];
  ...
  MeloraAudioSourceStreamObject? getStreamOfQuality(
    MeloraAudioSourceContainerPreset preset,
    int qualityIndex,
  ) {
    ...
    final exactMatch = sources.firstWhereOrNull(
      (source) {
        ...
        if (quality case MeloraAudioLosslessContainerQuality()) {
          return source.sampleRate == quality.sampleRate &&
              source.bitDepth == quality.bitDepth;
        } else {
          return source.bitrate ==
              (quality as MeloraAudioLossyContainerQuality).bitrate;
        }
      },
    );
    ...
    return sources.where((source) {
      ...
      if (quality is MeloraAudioLosslessContainerQuality) {
  ...
  MeloraAudioSourceContainerPreset? get qualityPreset {
