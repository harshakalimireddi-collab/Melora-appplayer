...
import 'package:melora/modules/player/player_queue.dart';
import 'package:melora/modules/player/sibling_tracks_sheet.dart';
import 'package:melora/components/adaptive/adaptive_pop_sheet_list.dart';
...
import 'package:melora/provider/sleep_timer_provider.dart';
...
class PlayerActions extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, ref) {
    ...
    final isLocalTrack = playlist.activeTrack is MeloraLocalTrackObject;
    ref.watch(downloadManagerProvider);
    final downloader = ref.watch(downloadManagerProvider.notifier);
    final isInQueue = useMemoized(() {
      if (playlist.activeTrack is! MeloraFullTrackObject) return false;
      final downloadTask =
      ...
      children: [
        if (showQueue)
          Tooltip(
            ...
            child: IconButton.ghost(
              icon: const Icon(MeloraIcons.queue),
        ...
        if (!isLocalTrack)
          Tooltip(
            ...
            child: IconButton.ghost(
              enabled: playlist.activeTrack != null,
              icon: const Icon(MeloraIcons.alternativeRoute),
              onPressed: () {
        ...
        if (!kIsWeb && !isLocalTrack)
          if (isInQueue)
            ...
            Tooltip(
              ...
              child: IconButton.ghost(
                icon: Icon(
                  isDownloaded ? MeloraIcons.done : MeloraIcons.download,
                ),
                onPressed: playlist.activeTrack != null
                    ? () => downloader.addToQueue(
                        playlist.activeTrack! as MeloraFullTrackObject)
                    : null,
              ),
            ),
